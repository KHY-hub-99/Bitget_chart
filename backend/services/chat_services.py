...
def convert_df_to_chart_data(df: pd.DataFrame, max_points: int = 5000):
    ...
    SMA 진입 규칙 및 시장 구조 강도(C 파트) 마킹을 포함하여 표준 컬럼을 완벽 준수합니다.
    """
    # [1] 데이터 슬라이싱 및 표준화
    ...
    # [3] 지표 데이터 그룹화 (Standard CamelCase 리스트 100% 반영)
    indicator_names = [
    ...
    # 레벨 확정 시점 포착을 위한 변화량 계산
    df_js['shl_diff'] = df_js['swingHighLevel'].diff()
    df_js['sll_diff'] = df_js['swingLowLevel'].diff()

    for _, row in df_js.iterrows():
        t = row['time']
        
        # A. 메인 진입 신호 (화살표 + 진입 경로 텍스트)
        if row.get('longSig') == 1:
            label = "LONG"
            if row.get('entrySmcLong') == 1: label = "LONG(SMC)"
            elif row.get('entrySmaLong') == 1: label = "LONG(SMA)"
            elif row.get('entryVwmaLong') == 1: label = "LONG(VWMA)"
            
            markers.append({"time": t, "position": "belowBar", "color": "#26a69a", "shape": "arrowUp", "text": label})
            
        elif row.get('shortSig') == 1:
            label = "SHORT"
            if row.get('entrySmcShort') == 1: label = "SHORT(SMC)"
            elif row.get('entrySmaShort') == 1: label = "SHORT(SMA)"
            elif row.get('entryVwmaShort') == 1: label = "SHORT(VWMA)"
            
            markers.append({"time": t, "position": "aboveBar", "color": "#ef5350", "shape": "arrowDown", "text": label})
        ...
        # C. 시장 구조 강도 마킹 (Strong/Weak 확정 시점)
        # 상단 레벨 확정
        if pd.notna(row.get('swingHighLevel')) and row.get('shl_diff') != 0:
            markers.append({
                "time": t, "position": "aboveBar", "color": "#878b94", 
                "shape": "circle", "text": str(row.get('topType', 'High'))
            })
        # 하단 레벨 확정
        if pd.notna(row.get('swingLowLevel')) and row.get('sll_diff') != 0:
            markers.append({
                "time": t, "position": "belowBar", "color": "#878b94", 
                "shape": "circle", "text": str(row.get('bottomType', 'Low'))
            })
